          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              ...
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 bg-[#1B5E20] rounded-full flex items-center justify-center text-white text-2xl font-bold">
                  {logement.proprio_prenom?.[0]?.toUpperCase()}
                </div>
                <div>
                  <p className="font-semibold text-gray-900">{logement.proprio_prenom} {logement.proprio_nom}</p>
                  <p className="text-gray-500 text-sm">{logement.nb_reservations_proprio} location{logement.nb_reservations_proprio > 1 ? 's' : ''} effectuée{logement.nb_reservations_proprio > 1 ? 's' : ''}</p>
                </div>
                {isAuthenticated && (
                  <button onClick={() => nav(`/dashboard/${user?.role}/messages`)}
                          className="ml-auto btn-outline py-2 px-4 text-sm">
                    Contacter
                  </button>
                )}
              </div>
